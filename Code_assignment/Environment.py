...
class StochasticWindyGridworld:
    ...
    def step(self, a:int):
        ...
        Returns the next state, the obtained reward, and a boolean whether the environment terminated 
            ...
            a (int): which action to take 
        '''
        self.agent_location += self.action_effects[a] # effect of action
        #if self.agent_location is outside boundaries, replace location with boundary value (so previous value)
        self.agent_location = np.clip(self.agent_location, (0,0), np.array(self.shape)-1) # bound within grid
        if np.random.rand() < self.wind_blows_proportion: # check whether wind triggers
            self.agent_location[1] += self.winds[self.agent_location[0]] # apply effect of wind
        self.agent_location = np.clip(self.agent_location, (0,0), np.array(self.shape)-1) # bound within grid
        ...
        # np.all checks whether both coordinates of agent match coordinates of goal_location
        if goal_present:
